            ...
            <tbody>
                ...
                @foreach ($list_zonasi_plp_2 as $key_plp_2 => $zonasi_plp_2)
                    <tr>
                        ...
                        @if ($role === 4)
                            <td style="width: 10%;">
                                <div class="btn-group-vertical">
                                    @if (empty($zonasi_plp_2->JointoPenilaian->JointoRkapAspek))
                                        <a href="{{ route('set.nilai.p2', ['id' => $zonasi_plp_2->JointoPenilaian->id_penilaian]) }}"
                                            class="btn btn-success btn-sm">Set
                                            Nilai</a>
                                    @else
                                        <a href="{{ route('edit.nilai.p2', ['id' => $zonasi_plp_2->JointoPenilaian->id_penilaian]) }}"
                        ...
                        @if ($role == 2)
                            <td style="width: 10%;">
                                @if ($zonasi_plp_2->JointoPenilaianDPl->jumlah_na == null && $zonasi_plp_2->JointoPenilaianDPl->huruf == null)
                                    <a href="{{ route('set.nilai.dpl.p2', ['id' => $zonasi_plp_2->id_zonasi]) }}"
                                            class="btn btn-success btn-sm w-100">Set Nilai</a>
                                @else
                                    <a href="{{ route('edit.nilai.dpl.p2', ['id' => $zonasi_plp_2->id_zonasi]) }}"
                                        class="btn btn-warning btn-sm w-100">Edit Nilai</a>
                                @endif
                            </td>
                        @endif
                    </tr>
