                ...
                @foreach ($list_zonasi_plp_2 as $item_plp_2 => $mhs_plp_2)
                    <tr>
                        ...
                        <th class="text-center">
                            @if (empty($mhs_plp_2->JointoRkapAspekDpl))
                                {{ '-' }}
                            @else
                                @foreach ($mhs_plp_2->JointoRkapAspekDpl as $item_n1 => $value_n1)
                        ...
                        <td class="text-center">
                            @if (empty($mhs_plp_2->status_laporan_terbaik))
                                <small>
                                    <a href="{{ route('video.terbaik', ['id' => $mhs_plp_2->id_penilaian_dpl]) }}" 
                                    class="text-danger">Set video dan laporan terbaik</a>
                                </small>
                            @else
                                @if ($mhs_plp_2->status_laporan_terbaik == 1)
                                    {{ 'Terbaik' }}
                                @endif
                            @endif
                        </td>
