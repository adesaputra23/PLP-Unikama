...
class MahasiswaController extends Controller
{
    ...
    {
        $list_mhs_plp_1 = Mahasiswa::where('jenis_plp', 1)->orderBy('created_at', 'desc')->get();
        $list_mhs_plp_2 = Mahasiswa::where('jenis_plp', 2)->orderBy('created_at', 'desc')->get();
    ...
    public function DownlaodFile($file_name)
    {
        $file = public_path()."/file_bukti_pembayaran"."/".$file_name;
        $headers = array('Content-Type: image/*',);
        return response()->download($file, $file_name, $headers);
    }

    public function Verifikasi($npm = null, $type = null)
    {
        try {
            $mhs = Mahasiswa::where('npm', $npm)->first();
            if ($type == 0) {
                $message = 'Data berhasil diverifikasi';
                $mhs->tgl_verifikasi = date('Y-m-d H:i:s');
            }else{
                $message = 'Verifikasi dibatalkan';
                $mhs->tgl_verifikasi = null;
            }
            $mhs->save();
            return redirect('data-mhs')->with('toast_success', $message);
        } catch (\Throwable $th) {
            return redirect('data-mhs')->with('toast_error', 'Error');
        }
    }
}
