...
class DPLController extends Controller
{
    ...
    public function PenilaianP1()
    {
        $user = Auth::user()->get_dpl->id_dpl;
            ...
            //     })->get();
            // $list_zonasi_plp_2 = ZonasiSekolah::with('JointoPenilaianDPl')->where('id_dpl', $user)
            //     ->whereHas('JointoMhs', function($query){
            //         $query->where('jenis_plp', 2);
            //     })->get();
            $list_zonasi_plp_2 = PenilaianDPl::whereHas('JointoZonasi', function($query) use ($user){
                    $query->where('id_dpl', $user);
            })->whereHas('JointoMhs', function($query){
                    $query->where('jenis_plp', 2);
            })->whereNotNull('jumlah_na')->whereNotNull('huruf')->get();
        return view(
            'dpl/penilaian',
                compact(
                    'list_zonasi_plp_2'
    ...
    public function SimpanNilaiDplP2(Request $request, $id)
    {
        ...
        $penilaian_DPL = PenilaianDPl::where('npm', $npm)->first();
        ...
        // if ($penilaian_DPL->save() == true) {
            ...
            try {
                ...
                $penilaian_DPL->jumlah_na = $na;
                $penilaian_DPL->huruf = $konfert_na;
                $penilaian_DPL->update();
    ...
    public function CetakNilaiDPL($id = null)
    {
        $data_user = PenilaianDPl::where('id_zonasi', $id)->first();
        // indikator
        $list_indikator_dpl_n1 = PnRkapIndikatorDPl::where('id_penilaian_dpl', $data_user->id_penilaian_dpl)
        ...
        $title = 'Penilaian DPL';
        $pdf = PDF::loadview('penilaian/cetak_penilaian_dpl', 
            compact(
                'title',
                'data_mhs',
                'list_prodi',
