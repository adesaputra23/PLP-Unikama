...
class PenilaianController extends Controller
{
    ...
    public function SimpantNilaiP1(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect('data-zonasi')->with('toast_success', 'Penilaian Berhasil di Buat');
        } catch (\Throwable $th) {
            //throw $th;
            DB::rollBack();
            return redirect('data-zonasi')->with('toast_error', 'Penilaian Gagal di Buat');
        }
    }
    ...
    public function SimpanNilaiP2(Request $request, $id)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect('data-zonasi')->with('toast_success', $missage);
            //code...
        } catch (\Throwable $th) {
            DB::rollBack();
            return redirect('data-zonasi')->with('toast_error', $missage);
            //throw $th;
        }
    ...
    public function CetakNilaiSekolah($id = null)
    {
        $penilaian = Penilaian::where('id_penilaian', $id)->first();
        $mhs = Mahasiswa::where('npm', $penilaian->npm)->first();
        if ($mhs->jenis_plp == 1) {
            $data_mhs = Penilaian::where('id_penilaian', $id)->first();
            $list_prodi = User::MAP_PRODI;
            $rkap_indikators = RkapIndikator::where('id_penilaian', $id)->get();
            $rkap_aspeks = RkapAspek::where('id_penilaian', $id)->get();
            $title = 'Penilaian Sekolah PLP I';
            $pdf = PDF::loadview('penilaian/cetak_penilaian_sekolah_p1', 
                compact(
                    'title',
                    'data_mhs',
                    'list_prodi',
            ...
            $rkap_aspek_n3 = RkapAspek::where('id_penilaian', $id)->where('id_pn_aspek', 6)->first();
            $rkap_aspek_n4 =RkapAspek::where('id_penilaian', $id)->where('id_pn_aspek', 7)->first();
            $title = 'Penilaian Sekolah PLP II';
            $pdf = PDF::loadview('penilaian/cetak_penilaian_sekolah_p2', 
                compact(
                    'title',
                    'penilaian',
                    'prodi',
        ...
        return redirect()->back()->with('toast_error', 'Error: Gagal Cetak pdf');   
    }
}
